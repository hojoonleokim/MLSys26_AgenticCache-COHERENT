...
class LLM:
	...
	def run(self, agent_node, chat_agent_info, current_room, next_rooms, all_landable_surfaces,
		...
		# Use predict prompt if specified
		if chat_agent_info.get('use_predict_prompt', False):
			# Replace the prompt file with predict version
			# e.g., quadrotor_prompt.txt -> quadrotor_predict_prompt.txt
			prompt_path = prompt_path.replace('_prompt.txt', '_predict_prompt.txt')
